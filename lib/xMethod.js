 ...
 */
module.exports = function xserviceMethod(spec) {
	...
	return function () {
		...
		var data = null
		var arg1 = args[args.length - 1]
		if (utils.isObject(arg1) || Array.isArray(arg1)) {
			data = args.pop()
		} else {
			data = {}
		}
		...
		for (var i = 0, l = urlParams.length; i < l; ++i) {
			...
			if (!arg) {
				...
				throw new Error('XService: I require argument "' + urlParams[i] + '", but I got: ' + arg)
			}
		...
		if (args.length) {
			throw new Error(
				'XService: Unknown arguments (' + args + '). Did you mean to pass an options object?'
			)
		}
